...
So here is what I did: for each game, I pulled the winning probabilities from fivethirtyeight and the corresponding odds from online bookmakers. Since bookmakers constantly update their odds (in order to have an edge over bettors), I always pulled both probabilities and odds the night before the match. With that in hand, I simulated three different betting stratgies. First, a bettor who always chooses the favorite to win. The second strategy is a contrarian bettor who always chooses the least likely outcome, which typically means betting on the underdog, but with high odds. Both ot these strategies never adjust the size of their stake, and always bet 10% of their intial bankroll (not their current bankroll). Lastly, I will simulate a bettor that looks at all probabilties, and odds, and then uses the [Kelly criterion](https://en.wikipedia.org/wiki/Kelly_criterion) for each bet. All bettors start out with the same bankroll of \$100.
...
```
We can see that basically two things happen: either the bettor loses the intial stake (which is always \$10), or wins a modest amount (since odds for favorites are tpyically not very high). After the group stage this bettor still has managed to accumulate some winnings.
...
The Kelly bettor realized the largest winnings from all three strategies by a wide margin. At the end of the group stage, the Kelly bettor accumulated about \$150 of winnnings. It will be intersting to see how this developed in the knock-out stage. 
