...
So here is what I did: for each game, I pulled the winning probabilities from fivethirtyeight and the corresponding odds from online bookmakers. Since bookmakers constantly update their odds (in order to have an edge over bettors), I always pulled both probabilities and odds the night before the match. With that in hand, I simulated three different betting stratgies. First, a bettor who always chooses the favorite to win, and always bets exactly \$10. The second strategy is a contrarian bettor who always chooses the least likely outcome, which typically means betting on the underdog, but with high odds, and always uses a \$10 bet. Lastly, I will simulate a bettor that looks at all probabilties, and odds, and then uses the [Kelly criterion](https://en.wikipedia.org/wiki/Kelly_criterion) for each bet. All bettors start out with the same bankroll of \$100.
