...
I will update this blog as the world cup progresses, but at this point, it's fair to say that this particular betting scheme is doing quite well. 
...
wcb <- wcb %>% mutate(posneg = factor(ifelse(wcb$`Total net`<0,"negative","positive"))) %>%  select(ID,Title,"Total net","Total stake",Bankrollkellyplot,posneg) %>% slice(1:42)
