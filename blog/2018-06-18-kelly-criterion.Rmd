...
So here is what I did: for each game, I pulled the winning probabilities from fivethirtyeight and the corresponding odds from online bookmakers. Since bookmakers constantly update their odds (in order to have an edge over bettors), I always pulled both probabilities and odds, the night before the match. With that in hand, I simulated three different betting stratgies, who all start out with \$100: first, a bettor who always chooses the favorite to win, and always bets exactly \$10. The second strategy is a contrarian bettor who always chooses the least likely outcome, which typically means betting on the underdog, but with high odds, and always uses a \$10 bet. Lastly, I will simulate a bettor that looks at all probabilties, and odds, and then uses the [Kelly criterion](https://en.wikipedia.org/wiki/Kelly_criterion) for each bet. 
...
Below I am showing a linegraph that shows how the initial bankroll of \$100 developed after each game. The y-axis shows relative bankroll (that means it simply tallies wins and losses and sets the zero point at the intial bankroll of \$100). The bars at the bottom of the graph show how much was won or lost in each bet, and the size of the point indicates the amount of total stake that was wagered on each game. 
...
```
We can see that basically two things happen: either the bettor loses the intial stake (which is always /$10), or wins a modest amount (since odds for favorites are tpyically not very high). After the group stage this bettor still has managed to accumulate winnings of about \$50. 

Comparing this with the contrarian bettor, we see a very different picture. This bettor is almost always losing the intital stake, and rarley wins. However the wins tend to be big. The largest gain was realized in the upset of South Korea over Germany. Without that single game, this bettor may be very close to bankruptcy already. 
...
The Kelly bettor realized the largest winnings from all three strategies by a wide margin. At the end of the group stage, the Kelly bettor accumulated about /$150 of winnnings, meaning a current bankroll of about \$250, starting with an intial bankroll of \$100. It will be intersting to see how this developed in the knock-out stage. 
